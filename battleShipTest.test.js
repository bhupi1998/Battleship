...
// noGoCoordinatesCalculator calculates all the coordinates that are off limits where no other object can be placed
test('testing if noGoCoordinatesCalculator returns expected coordinates', () => {
  const gameboard1 = new Gameboard();
  const realShip = new Ship(1);
  gameboard1.placeShips(4, 4, 270, realShip);
  expect(gameboard1.globalNoGoCoordinates).toStrictEqual([[3, 3], [4, 3], [5, 3], [3, 4], [4, 4], [5, 4], [3, 5], [4, 5], [5, 5]]);
});
