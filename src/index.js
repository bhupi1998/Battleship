...
export class Gameboard {
  ...
  // covered by the ship
  static shipCoordinateGenerator(xInitial, yInitial, orientation, length) {
    ...
    for (let i = length; i > 0; i -= 1) {
      const xNext = xInitial + (length - i) * xOrientationConst;
      const yNext = yInitial + (length - i) * yOrientationConst;
      tempArray.push([xNext, yNext]);
    }
    return { ...tempArray };
  }
  ...
  placeShips(xInit, yInit, orientation, shipObject) {
    ...
    shipSpotCoodinates = Gameboard.shipCoordinateGenerator(xInit, yInit, orientation, shipObject.length);
    console.log(shipSpotCoodinates.map((element) => [shipObject, ...element]));
    // this.hitPlaces.push(shipSpotCoodinates);
  }
}
